...
# Admin creds come da env/secrets (no hardcode!)
ADMIN_USER = read_secret("ADMIN_USER", "admin")
ADMIN_PASS_HASH = read_secret("ADMIN_PASS_HASH")  # preferito in prod
ADMIN_PASS = read_secret("ADMIN_PASS")            # solo in dev

# Aziende (configurabili via env/secrets)
...
def seed_demo_users():
    """Create/update ENI & Leonardo company users from env/secrets (hash > plain)."""
        ...
        if cid and eni_hash:
            ...
            if row:
                if row["password"] != eni_hash:
                    conn.execute(text("UPDATE company_user SET password=:p WHERE id=:id"),
                                 {"p": eni_hash, "id": row["id"]})
            else:
                conn.execute(text("""INSERT INTO company_user (company_id, email, password)
                                     VALUES (:cid, :e, :p)"""),
