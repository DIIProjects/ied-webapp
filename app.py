...
def get_active_event(conn):
    return conn.execute(text("SELECT id, name FROM event WHERE is_active=1 LIMIT 1")).mappings().first()
...
def is_checked_in(conn, event_id, student):
    return conn.execute(text("SELECT 1 FROM checkin WHERE event_id=:e AND student=:s LIMIT 1"),
                        {"e": event_id, "s": student}).first() is not None

def toggle_checkin(conn, event_id, student):
    if is_checked_in(conn, event_id, student):
        conn.execute(text("DELETE FROM checkin WHERE event_id=:e AND student=:s"), {"e": event_id, "s": student})
        return False
    else:
        conn.execute(text("INSERT INTO checkin (event_id, student, created_at) VALUES (:e,:s,:t)"),
                     {"e": event_id, "s": student, "t": datetime.utcnow().isoformat()})
        return True

def my_entry(conn, event_id, company_id, student):
    q = text("SELECT id, position, status FROM queue_entry WHERE event_id=:e AND company_id=:c AND student=:s AND status='queued' LIMIT 1")
    return conn.execute(q, {"e": event_id, "c": company_id, "s": student}).mappings().first()

def queue_len(conn, event_id, company_id):
    return conn.execute(text("SELECT COUNT(*) FROM queue_entry WHERE event_id=:e AND company_id=:c AND status='queued'"),
                        {"e": event_id, "c": company_id}).scalar_one()

def join_queue(conn, event_id, company_id, student):
    if my_entry(conn, event_id, company_id, student):
        return
    last = conn.execute(text("SELECT MAX(position) FROM queue_entry WHERE event_id=:e AND company_id=:c"),
                        {"e": event_id, "c": company_id}).scalar()
    next_pos = (last or 0) + 1
    conn.execute(text("INSERT INTO queue_entry (event_id, company_id, student, position, status, created_at) VALUES (:e,:c,:s,:p,'queued',:t)"),
                 {"e": event_id, "c": company_id, "s": student, "p": next_pos, "t": datetime.utcnow().isoformat()})

def leave_queue(conn, event_id, company_id, student):
    conn.execute(text("UPDATE queue_entry SET status='cancelled' WHERE event_id=:e AND company_id=:c AND student=:s AND status='queued'"),
                 {"e": event_id, "c": company_id, "s": student})

def roster_df(conn, event_id, company_id):
    rows = conn.execute(
        text("""
            SELECT position AS Posizione,
                   student  AS Studente,
                   created_at AS Inserito
            FROM queue_entry
            WHERE event_id = :e
              AND company_id = :c
              AND status = 'queued'
...
def add_company(conn, name):
    ...
    try:
        ...
        if ev:
            conn.execute(text("INSERT OR IGNORE INTO event_company (event_id, company_id) SELECT :e, id FROM company WHERE name=:n"),
                         {"e": ev["id"], "n": name})
